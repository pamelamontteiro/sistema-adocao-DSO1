...
class Registro():
    ...
    def registrar_doacao(self, data_de_doacao: date, animal: Animal, doador: Doador, motivo: str):
    ...
    def registrar_adocao(self, data_adocao: date, animal: Animal, adotante: Adotante, termo_assinado: bool):
        if (not adotante.data_nascimento < date.today().strftime('2006/%m/%d') # verificação se maior de idade (18 anos)
        ...
        and animal.__class__ == Cachorro and animal.tamanho == 'G'): #Com cachorro grande
            ...
            if tem_vacinas == 3: # verificação  se tem as vacinas necessárias
                ...
                if nao_doou: # verificaçao se nunca doou um animal, pois não pode adotar
                    #A adoção é liberada
                    self.__adocoes.append(Adocao(data_adocao, animal, adotante, termo_assinado))
